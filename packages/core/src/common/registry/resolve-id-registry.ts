...
export default class ResolveIdRegistry<TType, TToken extends { [key: string]: unknown }> {
    ...
    private _resolveToken(token: string, registeredTokens: string[]): string | undefined {
        ...
        for (const [, registeredToken] of Object.entries(registeredTokens)) {
            const resolverIds = this._decodeToken(registeredToken);

            for (const resolverId of Object.entries(resolverIds)) {
                const result = { token: '', matches: 0 };
